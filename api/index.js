...
app.get("/health", (req, res) => {
  ...
  res.json({ status: "ok", service: "Alrect Protect", ...(isProd ? {} : { version: "2.0.0", uptime: process.uptime() }) });
});
...
app.get("/script/:name", scriptLimiter, authMiddleware, (req, res) => {
  ...
  if (!/^[a-zA-Z0-9_\-\.]{1,64}$/.test(scriptName)) return res.status(400).json({ success: false, error: "INVALID_NAME" });
